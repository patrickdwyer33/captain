...
3. **Gather mission details** — ask the user for:
   ...
   > **Writing standard:** Apply `skills/writing-standard/RULES.md` when writing these fields. `title`, `goal`, `background`, `notes`, and `body` must be stateless (no "as discussed", no session labels, no first-person voice) and reflect current understanding only — no stale concepts or documented misunderstandings.
